...
/**
 * InterstellarSystems v5.6.0 - Physical Spline & Seamless Gradients
 * Fixes:
 * - Re-engineered Spline math for physically plausible, low-angle "Tangential Entry".
 * - Implemented seamless Color-Lerp function for smooth Orange-to-Blue transition.
 * - Refined volumetric fog layers for cohesive star-to-disk integration.
 */
export function createInterstellarSystems(scene, manager) {
    ...
    const createNebulaTexture = (type = 'disk') => {
        ...
        if (type === 'disk') {
            ...
            grad.addColorStop(0.6, 'rgba(0, 50, 150, 0.15)');
            grad.addColorStop(1, 'rgba(0, 0, 0, 0)');
            ctx.fillStyle = grad;
            ctx.fillRect(0, 0, size, size);
            ctx.globalCompositeOperation = 'lighter';
            for (let i = 0; i < 3000; i++) {
                const a = Math.random() * Math.PI * 2, r = Math.pow(Math.random(), 0.5) * size / 2;
                ctx.fillStyle = `rgba(180, 230, 255, ${Math.random() * 0.08})`;
            ...
            for (let i = 0; i < 1000; i++) {
                ...
                ctx.fillRect(Math.random() * size, Math.random() * size, Math.random() * 80 + 20, 1);
            }
        }
    ...
    const createEntity = (data, parentName, container, depth = 0, systemName) => {
        ...
        if (data.hasAccretionDisk) {
            ...
            const haze = new THREE.Mesh(new THREE.SphereGeometry(1, 64, 32), new THREE.MeshBasicMaterial({ color: 0x0055ff, transparent: true, opacity: 0.12, blending: THREE.AdditiveBlending, depthWrite: false }));
            haze.scale.set(diskSize * 1.25, diskSize * 0.22, diskSize * 1.25);
            const ring = new THREE.Mesh(new THREE.RingGeometry(baseScale * 0.1, diskSize, 128), new THREE.MeshBasicMaterial({ map: hqDiskTex, transparent: true, opacity: 0.85, blending: THREE.AdditiveBlending, side: THREE.DoubleSide, depthWrite: false }));
        ...
        if (data.orbit) {
            ...
            const o = new THREE.LineLoop(new THREE.BufferGeometry().setFromPoints(pts), new THREE.LineBasicMaterial({ color: 0xff3300, transparent: true, opacity: 0.08 }));
        ...
        update: (simSpeed, delta) => {
            ...
            gasStreams.forEach(gs => {
                ...
                if (s && t && disk) {
                    ...
                    // FIXED PHYSICAL PATH: Natural Low-Angle Insertion
                    const p1 = tip;
                    // p2 is the "momentum flare", p3 is the "tangential aligner"
                    const p2 = s.position.clone().add(dir.clone().multiplyScalar(dist * 0.35)).add(perp.clone().multiplyScalar(disk.outerRadius * 1.4));
                    const p3 = t.position.clone().add(dir.clone().multiplyScalar(-disk.outerRadius * 1.5)).add(perp.clone().multiplyScalar(disk.outerRadius * 1.2));
                    const p4 = t.position.clone().add(dir.clone().multiplyScalar(-disk.outerRadius * 0.9)).add(perp.clone().multiplyScalar(disk.outerRadius * 0.9));
                    ...
                    gs.group.children.forEach((mesh, idx) => {
                        ...
                        const colors = [];
                        const cS = new THREE.Color(0xff8833), cT = new THREE.Color(0x33b0ff);
                        ...
                        mesh.material.map.offset.y -= (1.8 + idx * 0.4) * simSpeed * delta;
